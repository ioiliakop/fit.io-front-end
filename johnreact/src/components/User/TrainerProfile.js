...
/**
 * prop matched from route uri
 * 
 * @param {Number} props.trainerId
 */
class TrainerProfile extends Component {
    ...
    // else we make necessary ajax calls to get them
    componentDidMount() {
    ...
    fetchTrainerReviews() {
        const url = 'http://localhost:8080/session/reviews-trainer/' + this.trainerId + '?start=1&end=9';
    ...
    render() {
        ...
        return (
            <div className="container complete-profile">
                <div className="container mb-4 p-4 col-9 bg-light shadow profile">
                    <div className="row mx-auto profile-header-info">
                        ...
                        <div className="container col-md-3 py-5 book-info">
                            <button className="btn btn-danger btn-block mt-auto">Book appointment</button>
                            <p className="text-middle text-center p-2 rounded mb-auto">Cost of session: <span className="text-danger">{this.state.trainer.price}&euro;</span></p>
                        </div>
                    </div>
                ...
                {/* Only if the trainer has reviews will the next secton be rendered */}
