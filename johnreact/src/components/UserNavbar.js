...
function NavLink(props) {    
    if (props.to === props.location) {
        return <Link className="list-group-item list-group-item-action active" to={props.to}>{props.label}</Link>
    }
    else {
        return <Link className="list-group-item list-group-item-action" to={props.to}>{props.label}</Link>
    }    
}
        ...
        return (
            <div className="container">
                <div className="list-group list-group-horizontal-sm">
                    <NavLink label="My Training Sessions" to="/temp1" location={this.props.location.pathname} />
